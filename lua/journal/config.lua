...
local function convert_strings_to_functions(entry_config, include_keys)
	for key, value in pairs(entry_config) do
		...
		if is_dict_like(entry_config[key]) then
			convert_strings_to_functions(entry_config[key], include_keys)
		else
			if type(value) == "string" then
				entry_config[key] = function() return value end
			end
		end
...
M.setup = function(user_config)
	...
	convert_strings_to_functions(defaults.journal, { 'template', 'format' })
end
